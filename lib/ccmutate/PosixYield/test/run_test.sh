...
testLibName="mutate_PosixYield.so"
libraryName="PosixYield"
...
$opt -debug -load $llvmlibdir/$testLibName -$libraryName -rm -pos=0 <test.bc >out01.bc
$llvmdis <out01.bc >out01.ll
echo "END TEST"
echo " "

echo "BEGIN TEST: Modify position 1, output to out10.bc"
$opt -debug -load $llvmlibdir/$testLibName -$libraryName -rm -pos=1 <test.bc >out10.bc \
...
$opt -debug -load $llvmlibdir/$testLibName -$libraryName -rm -pos=1,0 <test.bc >out11.bc
...
$opt -debug -load $llvmlibdir/$testLibName -$libraryName -rm -pos=0,0 <test.bc >out01_0.bc
$llvmdis <out01_0.bc >out01_0.bc
echo "END TEST"
echo " "

echo "BEGIN TEST: Specify verbose and rmmode"
$opt -debug -load $llvmlibdir/$testLibName -$libraryName -rm -verbose <test.bc >out01_0.bc
echo "END TEST"
echo " "
